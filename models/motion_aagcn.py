...
from torch.nn import Sequential, Linear, Dropout, BatchNorm1d, Sigmoid, ReLU
...
from models.m2d import Graph
class Model(nn.Module):
    def __init__(self, num_class=10, num_point=17, num_person=1, graph=Graph, graph_args={'labeling_mode': 'spatial'}, in_channels=6,
        ...
        # self.l2mfcc = nn.Sequential(
        #     nn.Linear(64, 80),
        #     nn.ReLU()
        # )
        self.fc = nn.Linear(256, num_class)
        # self.fc_motion = Sequential(
        #     Linear(256, 1152, bias=False),
        #     Dropout(0.25),
        # )
    ...
    def forward(self, x):
        ...
        # x = self.l4(x)
        # mid_out = x[:]
        # mid_out = mid_out.mean(3)
        # mid_out = self.l2mfcc(mid_out.transpose(1,2))
        x = self.l5(x)
        x = self.l6(x)
        # x = self.l7(x)
        x = self.l8(x)
